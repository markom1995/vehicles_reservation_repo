...
public class UserRepositoryImpl implements UserRepositoryCustom {
    ...
    private static final String SQL_GET_BY_COMPANY_ID_AND_DELETED = "SELECT u.id, u.username, u.first_name, u.last_name, r.name as role_name FROM user u JOIN role r ON u.role_id=r.id WHERE u.company_id=? AND u.deleted=? AND u.active=?";
    ...
    public List<UserRole> getByCompanyIdAndDeletedAndActive(Integer companyId, byte deleted, byte active) {
        return entityManager.createNativeQuery(SQL_GET_BY_COMPANY_ID_AND_DELETED, "UserRoleMapping").setParameter(1, companyId).setParameter(2, deleted).setParameter(1, companyId).setParameter(3, active).getResultList();
    }
}
