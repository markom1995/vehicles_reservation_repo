...
public class VehicleRepositoryImpl implements VehicleRepositoryCustom {
    ...
    private static final String SQL_GET_EXTENDED_BY_ID = "SELECT v.id, v.license_plate, v.vehicle_model_id, v.year, v.engine, v.fuel, v.photo, v.deleted, v.location_id, v.company_id, vman.name as manufacturer_name, vmod.name as model_name, l.longitude as longitude, l.latitude as latitude FROM vehicle v JOIN vehicle_model vmod ON v.vehicle_model_id=vmod.id JOIN vehicle_manufacturer vman ON vmod.vehicle_manufacturer_id=vman.id JOIN location l ON v.location_id=l.id WHERE v.id=?";
    ...
    @Override
    public VehicleLocationVehicleModelVehicleManufacturer getExtendedById(Integer id) {
        List<VehicleLocationVehicleModelVehicleManufacturer> vehicles = entityManager.createNativeQuery(SQL_GET_EXTENDED_BY_ID, "VehicleLocationVehicleModelVehicleManufacturerMapping").setParameter(1, id).getResultList();
